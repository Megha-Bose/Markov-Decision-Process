...
def find_east(state):
    ...
    val = 0
    if mm_state == 'D':
        val += 1.0 * 0.8 * discount *  prev_utilities[('W', mat, arrow, 'D', health)][0]
        val += 1.0 * 0.2 * discount *  prev_utilities[('W', mat, arrow, 'R', health)][0]
    elif mm_state == 'R':
        val += 1.0 * 0.5 * (discount * prev_utilities[('E', mat, 0, 'D', min(100, health+25))][0] - 40)
        val += 1.0 * 0.5 * discount *  prev_utilities[('W', mat, arrow, 'R', health)][0]
    val += step_cost
    if val > best:
        best_move = "LEFT"
        best = val
